...
With multi-functions, the computed derivative have constant magnitude.
...
Since any eigenvector is subject to an arbitrary change in sign, we choose, at each vertex,
the eigenvector which is closest to the one corresponding to the previous vertex.
...
In the above examples, the jump of the vector field after a complete loop was a change in sign,
which can be viewed as a $ 180^\circ $ rotation.
We can choose any angle instead of $ 180^\circ $.
By doing this, we can no longer view this vector field as the eigenvector of a matrix.
...
Figure \ref{\numb section 7.\numb fig 23} shows how this vector field looks like (with a
coefficient of one-third).
