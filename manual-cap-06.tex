...
After declaring a finite element of type Lagrange, the process of creating new cells
(vertices, in this specfic case) will be different.
Space will be reserved for a {\small\tt size\_\,t} value for each vertex, and
this value can be used for numbering vertices.
We will present this approach in paragraph \ref{\numb section 6.\numb parag 4}.
For now, we prefer to build by hand a {\small\tt map} for numbering vertices.
...
In previous examples we have built a numbering {\small\tt map}.
In this paragraph we show how to use instead the automatic numbering associated to the
finite element itself.
After declaring a finite element of type Lagrange, the process of creating new cells
(vertices, in this specfic case) will be different.
A new function will be added to the list of functions {\small\tt\verm{Cell}::init\_\,pos\_\,cell[0]},
invoked by {\small\tt\verm{Cell}::Core} constructors (see paragraph
\ref{\numb section 11.\numb parag 10}).
Space will be reserved for a {\small\tt size\_\,t} value for each vertex, and
this value can be used for numbering vertices, as shown below.
...
request and other vertices used internally by {\maniFEM} which are invisible to the user.
...
have zero entries.
However, the solver of the linear system will not work because the matrix is singular.
