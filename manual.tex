...
After this introduction, you can find three lists showing {\maniFEM}'s competitors,
...
\section*{Similar products (competitors)}
...
\noindent In progressive mesh generation, change
{\small\tt\textcolor{tag}{tag}::inherent\_\,orientation} into
{\small\tt\textcolor{tag}{tag}::orientation,} {\small\tt\textcolor{tag}{tag}::inherent} etc.

\noindent Introduce constructor {\small\tt\verm{Mesh} \azul{msh} ( \textcolor{tag}{tag}::import, tag::msh, \verde{"filename.msh"})}.

\noindent Change method name from {\small\tt\verm{Mesh}::export\_\,msh} to
...
\hglue 15mm see also intrinsic orientation, inherent orientation, random orientation
...
\noindent
writing a mesh to a file, see {\small\tt\verm{Mesh}::export}
