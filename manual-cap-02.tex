...
by propagating a front of segments and progressively adding triangular cells, one by one.
...
In {\maniFEM} there is a global variable {\small\tt\verm{Manifold}::working}.
Any {\small\tt\verm{Manifold}} constructor sets this variable to the manifold
being defined.
Subsequently, most {\small\tt\verm{Mesh}} constructors, when building inner vertices,
...
The projection operation is a method of the {\small\tt\verm{Manifold}} itself.
...
There is also a {\small\tt\verm{Cell}} constructor with {\small\tt\textcolor{tag}{tag}::of\_\,coords}
and {\small\tt\textcolor{tag}{tag}::project}.
This constructor first sets the values of {\small\tt x} and {\small\tt y} and then performs,
under the curtains, a projection on the current working {\small\tt\verm{Manifold}} :

\begin{Verbatim}[commandchars=\\\{\},formatcom=\small\tt,baselinestretch=0.94]
   \verm{Cell} \azul{P} ( \textcolor{tag}{tag}::vertex, \textcolor{tag}{tag}::of_coords, \{ \laranja{0.6}, \laranja{2.1} \}, \textcolor{tag}{tag}::project );
\end{Verbatim}

Also, the {\small\tt\verm{Mesh}} constructor with
...
Again, when we define individual points {\small\tt\azul E}, {\small\tt\azul N}, {\small\tt\azul W},
{\small\tt\azul S} and {\small\tt\azul{up}} we must be careful to provide coordinates on the
...
\begin{Verbatim}[commandchars=\\\{\},formatcom=\small\tt,frame=single,
   label=parag-\ref{\numb section 2.\numb parag 12}.cpp,rulecolor=\color{moldura},
   baselinestretch=0.94,framesep=2mm]
   \verm{Manifold} \azul{four_points} ( \textcolor{tag}{tag}::intersect, parab_vert, parab_horiz );
\end{Verbatim}
...
Here is a three-dimensional version of the diamond in paragraph \ref{\numb section 2.\numb parag 11}.
...
\begin{Verbatim}[commandchars=\\\{\},formatcom=\small\tt,frame=single,
   ...
   \{  \verm{Cell} \azul{P} = *it;
      ...
      if ( z(P) > \laranja{1.3} )
      \{  x(P) /= \laranja{1.} + \laranja{300.} * std::pow ( z(P) - \laranja{1.3}, \laranja{3.} );
         y(P) /= \laranja{1.} + \laranja{300.} * std::pow ( z(P) - \laranja{1.3}, \laranja{3.} );
         z(P) *= \laranja{1.} + \laranja{10.} * ( z(P) - \laranja{1.3} ) * ( z(P) - \laranja{1.3} );  \}
