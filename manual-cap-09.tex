...
cells and meshes are created by declaring them as {\small\tt \verm{Cell}} or {\small\tt \verm{Mesh}} objects
...
when you declare a segment {\small\tt \verm{Mesh}} (you must provide a negative {\small\tt \verm{Cell}} as
starting point).
...
Note that the above code does not work as it is (it only works for certain meshes upon
...
lower dimension.
Thus, if {\small\tt cube} is a {\small\tt \verm{Cell}} object modelling a regular hexahedron,
then {\small\tt cube.boundary()} is a {\small\tt \verm{Mesh}} object modelling a
...
{\small\tt it.advance()}.
We have also kept the notation {\small\tt *it} for dereferencing a {\small\tt \verm{CellIterator}};
this operation returns a reference to a {\small\tt \verm{Cell}} object.
