...
by adding a {\small\tt \verm{tag}::force\_\,positive} as in
...
{\small\tt \verm{tag}::cells\_\,of\_\,dim,}\hskip1pt{\small\tt 0}\hskip1pt{\small\tt )}},
while {\small\tt msh.number\_\,of}\hskip2pt{\small\tt
(}\hskip2pt{\small\tt \verm{tag}::segments}\hskip2pt{\small\tt )} is equivalent to
{\small\tt msh.number\_\,of}\hskip2pt{\small\tt (}\hskip2pt{\small\tt \verm{tag}::cells\_\,of\_\,dim,}\hskip2pt{\small\tt 1}\hskip2pt{\small\tt )}.
...
{\small\tt \verm{tag}::start\_\,at} followed by a second argument of type {\small\tt\verm{Cell}}.
...
{\small\tt \verm{tag}::force\_\,positive}, as shown in paragraph \ref{\numb section 9.\numb parag 5}.
...
As explained in paragraph \ref{\numb section 9.\numb parag 10},
de-referencing a {\small\tt \verm{CellIterator}} does not produce a new cell,
just provides access to a previously built cell.
...
{\small\tt(} {\small\tt\verm{tag}::over\_\,vertices} {\small\tt)} will obey to
...
{\small\tt \verm{tag}::require\_\,order} :
...
you want to iterate over their positive counterparts, use the {\small\tt \verm{tag}::force\_\,positive}.
...
\begin{figure}[ht] \centering
  ...
  \psfrag{C}{\tt\textcolor{textindraw}{C}}
  \psfrag{D}{\tt\textcolor{textindraw}{D}}
  \psfrag{hex_1}{\tt\textcolor{textindraw}{hex\_\,1}}
  \psfrag{hex_2}{\tt\textcolor{textindraw}{hex\_\,2}}
...
{\small\tt \verm{tag}::may\_\;not\_\,exist} and then the cell's method {\small\tt exists} :
...
If you want positive segments, add a {\small\tt\verm{tag}::force\_\,positive} before
...
Note that, if you provide a {\small\tt\verm{tag}::force\_\,positive}, it makes no difference
...
can always add the {\small\tt\verm{tag}::force\_\,positive} :
...
tags like {\small\tt\verm{tag}::backwards} or {\small\tt\verm{tag}::require\_\,order}.
Also, the {\small\tt reset} method accepts the {\small\tt\verm{tag}::start\_\,at} followed
...
with the notion of co-dimension, you can add the {\small\tt\verm{tag}::require\_\,order}
...
As a general rule, de-referencing a {\small\tt\verm{CellIterator}} does not build a new
...
you can provide the {\small\tt\verm{tag}::do\_\,not\_\,build\_\,cells} as an argument
...
De-referencing a {\small\tt\verm{CellIterator}} returns (a new wrapper for) an existing cell;
