...
This is so because the {\small\tt \verm{Mesh}} constructor with {\small\tt \textcolor{tag}{tag}::quadrangle} treats the
...
{\small\tt \textcolor{tag}{tag}::random\_\,orientation} as last argument to the {\small\tt \verm{Mesh}} constructor.
...
{\small\tt \textcolor{tag}{tag}::intrinsinc\_\,orien\-tation} as argument to the {\small\tt \verm{Mesh}} constructor.
...
{\small\tt \textcolor{tag}{tag}::random\_\,orientation} as argument to the {\small\tt \verm{Mesh}} constructor.
We can enforce the requirement of finding the inherent orientation by providing a
{\small\tt \textcolor{tag}{tag}::inherent\_\,orientation} as argument to the {\small\tt \verm{Mesh}} constructor;
...
{\small\tt \textcolor{tag}{tag}::random\_\,orientation} as a last argument to the {\small\tt \verm{Mesh}} constructor,
...
<\small\tt \laranja0\laranja.,> <\small\tt \laranja0\laranja.,> <\small\tt \laranja-\laranja1\laranja.> <\small\tt }>\catcode`<=12\catcode`>=12\catcode`{=1\catcode`}=2, we will obtain
...
%\begin{Verbatim}[commandchars=\\\{\},formatcom=\small\tt,
%   baselinestretch=0.94,framesep=2mm                      ]
%   \verm{Mesh} diamond ( \textcolor{tag}{tag}::progressive, \textcolor{tag}{tag}::boundary, NW, WS, SE, EN,
%                  \textcolor{tag}{tag}::desired_length, 1.5 );
%\end{Verbatim}
...
This happens if we choose {\small\tt\azul{rc} = 0.5} in paragraph \ref{\numb section 3.\numb parag 18}
or \ref{\numb section 3.\numb parag 19}.
