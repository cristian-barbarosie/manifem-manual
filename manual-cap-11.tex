...
by the user.
Examples are : {\small\tt\verm{smooth\_min}}, {\small\tt\verm{power}}, {\small\tt\verm{sin}}.
...
There are also functions like {\small\tt \verm{smooth\_min}} and also overloaded versions
...
discrete. For instance, if we wrote {\small\tt\verm{Tag}::divided\_in},
the reader's eye would catch {\small\tt\verm{Tag}} much before {\small\tt divided\_in};
thus, {\small\tt\verm{tag}::divided\_in} is more readable.
...
through the statement {\small\tt \verm{Mesh}::set\_max\_dim (}some-integer{\small\tt )}.
...
the statement {\small\tt \verm{Mesh}::set\_max\_dim(2)}.
...
At low level, cells and meshes are built by methods {\small\tt \verm{Cell}::glue\_on\_bdry\_of},
{\small\tt \verm{Cell}::cut\_from\_bdry\_of}, {\small\tt \verm{Cell}::add\_to\_mesh} and
{\small\tt \verm{Cell}::remove\_from\_mesh}.

The inlined method {\small\tt \verm{Cell}::glue\_on\_bdry\_of} is used for sticking a new
{\small\tt face} on the
boundary of an existing cell {\small\tt cll} (whose boundary is incomplete).
It simply calls the inlined method \ {\small\tt \verm{Cell}::Core::glue\_on\_bdry\_of}
which in turn calls the virtual method {\small\tt cll->glue\_on\_my\_bdry(face)}.
Different methods {\small\tt \verm{Cell}::***::glue\_on\_my\_bdry} exist for different kinds of
...
then calls the inlined method {\small\tt \verm{Cell}::Positive::NotVertex::glue\_common}.

Method {\small\tt \verm{Cell}::Core::add\_to\_bdry} is virtual so it does different things for
...
{\small\tt \verm{Cell}::Positive::meshes}, {\small\tt \verm{Cell}::Positive***::meshes\_same\_dim} and
...
coded in the map {\small\tt \verm{Cell}::Core:: ::cell\_behind\_within}.

The inlined method {\small\tt \verm{Cell}::Positive::NotVertex::glue\_common} deals with
...
we call the inlined method {\small\tt \verm{Cell}::add\_to} which calls the virtual method
{\small\tt \verm{Cell}::Core::add\_to\_mesh} which is similar to {\small\tt \verm{Cell}::Core::add\_to\_bdry}
described above.
...
with methods replaced by {\small\tt \verm{Cell}::cut\_from\_bdry\_of},
{\small\tt \verm{Cell}::remove\_from\_mesh},
{\small\tt \verm{Cell}::Core::cut\_from\_bdry\_of},
{\small\tt \verm{Cell}::Core::cut\_from\_my\_bdry},
{\small\tt \verm{Cell}::Core::remove\_from\_bdry}, {\small\tt \verm{Cell}::Core::remove\_from\_mesh},
{\small\tt \verm{Mesh}::Core:remove\_***}, {\small\tt break\_deep\_connections},
{\small\tt \verm{Cell}::Positive::NotVertex::cut\_common}.


                 %----------------------%
\section{~~\cinza{About {\tt init\_cell}} (outdated)}\label{\numb section 11.\numb parag 10}
                 %----------------------%

The {\small\tt \verm{Cell}} class has static attributes {\small\tt init\_cell},
...
For instance, {\small\tt \verm{Manifold}::coordinate\_system} inserts into the list
{\small\tt \verm{Cell}::init\_cell[0]} a call to {\small\tt \verm{Mesh}::prescribe\_on} which in turn calls
{\small\tt \verm{FunctionOnMesh}::prescribe\_on} in order to prepare the ground for instructions like
...
adds a call to\break {\small\tt\verm{FiniteElement}::Lagrange::Q1::enumerate\_new\_vertex} to
the list {\small\tt\verm{Cell}::init\_cell[0]}.
...
Consider using the {\small\tt \verm{tag}::force\_positive}.
See paragraphs \ref{\numb section 9.\numb parag 5} and \ref{\numb section 9.\numb parag 6}.
...
{\small\tt \verm{Mesh}::set\_max\_dim} ?
Along your program, you may have different maximum topological dimensions (that is,
you may use {\small\tt \verm{Mesh}::set\_max\_dim} several times) but, at each moment,
