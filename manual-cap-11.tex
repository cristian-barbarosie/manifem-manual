...
For instance, you will have to write {\small\tt\verm{maniFEM}::Mesh::Iterator} instead of
{\small\tt\verm{Mesh}::Iterator}, and so on.
...
{\small\tt\verm{Mesh}::Iterator}, {\small\tt\verm{Manifold}::Action},
many {\small\tt\verm{tag}}s (see paragraph \ref{\numb section 11.\numb parag 3}).
...
\ {\small\tt\verm{Mesh}::Iterator::Over::SegsOfPosLoop::Positive},
...
Examples are : {\small\tt\verm{min}}, {\small\tt\verm{max}}, {\small\tt\verm{smooth\_\,min}},
{\small\tt\verm{smooth\_\,max}}, {\small\tt\verm{power}}, {\small\tt\verm{abs}}, {\small\tt\verm{sign}},
{\small\tt\verm{sin}}, {\small\tt\verm{cos}}.
...
\verm{blue} : {\small\tt\verm{Cell}}, {\small\tt\verm{Mesh}},
...
the even more rarely needed {\small\tt\verm{MetricTree}}.
...
The word {\small\tt\verm{tag}} deserves a special treatment.
...
Numeric constants are \laranja{orange}.
...
For instance, in the code excerpt below five {\small\tt\verm{Mesh}::Iterator}s are defined
...
There is a drawback to using {\small\tt\textcolor{tag}{tag}}s instead of choosing different names
for functions : compilation error messages may become quite long.
...
{\small\tt\verm{tag}} but beware, it has many names which may conflict
...
iterators, fields, functions, manifolds, integrators, finite elements.
...
{\small\tt\verm{Mesh}::Iterator}s, {\small\tt\verm{Manifold}}s,
...
A garbage collector is implemented; many of these classes inherit from
...
{\small\tt\verm{FiniteElements}} and {\small\tt\verm{Integrators}} depend on each other
in a complicated manner.
The user may choose to declare an {\small\tt\verm{Integrator}};
a {\small\tt\verm{FiniteElement}} will be built behind the scenes;
this happens in paragraphs \ref{\numb section 1.\numb parag 1} and \ref{\numb section 6.\numb parag 2}.
Or, they may choose to declare a {\small\tt\verm{FiniteElement}} and later build an
{\small\tt\verm{Integrator}} by invoking the method {\small\tt\verm{FiniteElement}::set\_\,integrator};
this happens in paragraphs \ref{\numb section 6.\numb parag 3} -- \ref{\numb section 6.\numb parag 9}.
In the former case, the {\small\tt\verm{Integrator}} is responsible for keeping the
{\small\tt\verm{FiniteElement}} alive and later for destroying it.
In the latter case, it is the other way around.
This is why we have implemented a dedicated garbage collector which behaves on most occasions
like a {\small\tt shared\_\,ptr} but sometimes like a {\small\tt weak\_\,ptr}.

Cells and meshes depend on each other in a complicated manner, too.
...
This has to do with the {\small\tt\verm{MetricTree}} (described in
...
of {\small\tt\verm{Mesh}::Core}s.

Negative meshes are temporary wrappers built on-the-fly.
Their wrapper contains a pointer to a positive mesh core.
...
There are also {\small\tt STSI} meshes; the name means ``self-touching or self-intersecting'' meshes.
...
the statement {\small\tt\verm{Mesh}::set\_\,max\_\,dim (\laranja{2})}.
         ...
         tri->boundary->add_***_seg ( seg, \textcolor{tag}{tag}::mesh_is_bdry )
            make_deep_connections_1d ( seg, tri->boundary, \textcolor{tag}{tag}::mesh_is_bdry )
               add_link_same_dim ( seg, tri->boundary )
                  tri->boundary->cells[\laranja{1}] .push ( seg )  \cinza{// \mbox{\fontfamily{helvetica}\selectfont{}if} tri->boundary \mbox{\fontfamily{helvetica}\selectfont{}is} Fuzzy}
                  ...
                  for \azul{msh} in \{\mbox{\fontfamily{helvetica}\selectfont\,meshes of dimension} \azul{d} >= \laranja{2} \mbox{\fontfamily{helvetica}\selectfont{}containing} tri\,\}
                     add_link ( seg, msh, *, * )
                        if seg \mbox{\fontfamily{helvetica}\selectfont{}already belongs} to msh
                           ...
                           msh->cells[\laranja{1}] .push ( seg )  \cinza{// \mbox{\fontfamily{helvetica}\selectfont{}if} msh \mbox{\fontfamily{helvetica}\selectfont{}is} Fuzzy}
                           seg->meshes[d] [ msh ] = ( *, *, wh )
                  ...
                  link_face_to_msh ( ver, seg, tri->boundary, *, * )
                     add_link ( ver, tri->boundary, *, * )
                        if ver \mbox{\fontfamily{helvetica}\selectfont{}already belongs to} tri->boundary
                           ...
                           tri->boundary->cells[\laranja{0}] .push ( ver )  \cinza{// \mbox{\fontfamily{helvetica}\selectfont{}if} Fuzzy}
                     ...
                     for \azul{msh} in \{\mbox{\fontfamily{helvetica}\selectfont\,meshes of dimension} \azul{d} >= \laranja{2} \mbox{\fontfamily{helvetica}\selectfont{}containing} tri\,\}
                        add_link ( ver, msh, *, * )
                           if ver \mbox{\fontfamily{helvetica}\selectfont{}already belongs to} msh
                              ...
                              msh->cells[\laranja{0}] .push ( ver )  \cinza{// \mbox{\fontfamily{helvetica}\selectfont{}if} msh \mbox{\fontfamily{helvetica}\selectfont{}is} Fuzzy}
                              ver->meshes[d] [ msh ] = ( *, *, wh )
                  ...
                  unlink_face_from_msh ( ver, seg, tri->boundary, *, * )
                     remove_link ( ver, tri->boundary, *, * )
                        ...
                        if \mbox{\fontfamily{helvetica}\selectfont{}both counters zero}
                           ...
                           tri->boundary->cells[\laranja{0}] .remove ( ver )  \cinza{// \mbox{\fontfamily{helvetica}\selectfont{}if} Fuzzy}
                     ...
                     for \azul{msh} in \{\mbox{\fontfamily{helvetica}\selectfont\,meshes of dimension} \azul{d} >= \laranja{2} \mbox{\fontfamily{helvetica}\selectfont{}containing} tri\,\}
                        remove_link ( ver, msh, *, * )
                           ...
                           if \mbox{\fontfamily{helvetica}\selectfont{}both counters zero}
                              ...
                              msh->cells[\laranja{0}] .remove ( ver )  \cinza{// \mbox{\fontfamily{helvetica}\selectfont{}if} msh \mbox{\fontfamily{helvetica}\selectfont{}is} Fuzzy}
                  ...
                  for \azul{msh} in \{\mbox{\fontfamily{helvetica}\selectfont\,meshes of dimension} \azul{d} >= 2 \mbox{\fontfamily{helvetica}\selectfont{}containing} tri\,\}
                     remove_link ( seg, msh, *, * )
                        ...
                        if \mbox{\fontfamily{helvetica}\selectfont{}both counters zero}
                           ...
                           msh->cells[\laranja{1}] .remove ( seg )  \cinza{// \mbox{\fontfamily{helvetica}\selectfont{}if} msh \mbox{\fontfamily{helvetica}\selectfont{}is} Fuzzy}
               remove_link_same_dim ( seg, tri->boundary )
                  tri->boundary->cells[\laranja{1}] .remove ( seg )  \cinza{// \mbox{\fontfamily{helvetica}\selectfont{}if} tri->boundary \mbox{\fontfamily{helvetica}\selectfont{}is} Fuzzy}
...
For \ instance, \ the method {\small\tt\verm{FiniteElement}::build\_\,global\_\,numbering}
\ builds \ a \ new {\small\tt\verm{Cell}:: ::Numbering::Field} whose constructor
adds a call to {\small\tt\verm{Cell}::Numbering::Field::set\_\,and\_\,increment} to
the list {\small\tt\verm{Cell}::init\_\;pos\_\,cell[\laranja{0}]}.
...
I do not use the {\small\tt auto} keyword in the source code of \maniFEM.
I find that explicitly declaring the type of each variable makes the code more readable,
at the price of increasing its size a little.

{\small\tt\verm{Mesh}::Iterator}s obey to syntactic rules slightly different from the conventions for
...
Sometimes, an {\small\tt\verm{Integrator}} is declared but never formally used,
the integration command being delegated to the associated {\small\tt\verm{FiniteElement}}.
...
The source files of {\maniFEM} are grouped in the directory {\small\tt src};
